...
function loadBilling() {
    try {
        const saved = localStorage.getItem('sf_billing');
        if (saved) {
        ...
        try { localStorage.removeItem('sf_billing'); } catch (_) { }
    }
    ...
    try { localStorage.setItem('sf_billing', JSON.stringify(billing)); } catch (_) { }
    return billing;
}

function saveBilling(billing) {
    try {
        localStorage.setItem('sf_billing', JSON.stringify(billing));
    } catch (e) {
...
function loadData() {
    ...
    const existingBilling = localStorage.getItem('sf_billing');
    if (!existingBilling) {
        try {
            const demoBilling = {
                plan: 'pro',
            ...
            localStorage.setItem('sf_billing', JSON.stringify(demoBilling));
